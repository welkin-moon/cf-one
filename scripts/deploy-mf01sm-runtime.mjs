...
try {
  ...
  if (!source.includes('3.1.1') || !source.includes('response_quality_detail') || !source.includes('CF-Connecting-IP')) {
    throw new Error('mf01sm runtime bundle is missing v3.1.1 observability markers; refusing to deploy.');
  }
  ...
  const metadata = {
    ...
    annotations: {
      'workers/tag': `mf01sm-v3.1.1-${Date.now().toString(36)}`,
      'workers/message': 'mf01sm v3.1.1 observability runtime'
    }
  };
  ...
  const deployment = await api(`/accounts/${accountId}/workers/scripts/${scriptName}/deployments`, {
    ...
    body: JSON.stringify({
      ...
      annotations: { 'workers/message': 'mf01sm v3.1.1 observability runtime' }
    })
  });
  if (!deployment?.id) throw new Error('Cloudflare created no mf01sm runtime deployment id.');
  console.log(`mf01sm runtime ${version.id} deployed at 100%; D1-primary/KV-fallback storage is active.`);
} finally {
