...
const V33_JS = String.raw`<script id="mf01sm-v33-selfid-js">(()=>{function setup(tagsId,attr,proxyId,inputId,storeKey){const root=document.getElementById(tagsId),proxy=document.getElementById(proxyId),input=document.getElementById(inputId);if(!root||!proxy)return;const buttons=[...root.querySelectorAll('['+attr+']')],selected=new Set();const sync=()=>{const other=(input?.value||'').trim();const parts=[...selected];if(other)parts.push('其他:'+other.slice(0,28));if(storeKey){state.selfLikert[storeKey+'_tags']=[...selected];state.selfLikert[storeKey+'_other']=other.slice(0,28);}proxy.dataset.value=parts.join('｜').slice(0,80);proxy.click();};buttons.forEach(btn=>btn.addEventListener('click',()=>{const value=btn.getAttribute(attr)||'',none=btn.dataset.noLabel==='1';if(none){selected.clear();buttons.forEach(x=>{x.classList.remove('active');x.setAttribute('aria-pressed','false');});selected.add(value);}else{for(const x of buttons){if(x.dataset.noLabel==='1'){selected.delete(x.getAttribute(attr)||'');x.classList.remove('active');x.setAttribute('aria-pressed','false');}}if(selected.has(value))selected.delete(value);else selected.add(value);}for(const x of buttons){const active=selected.has(x.getAttribute(attr)||'');x.classList.toggle('active',active);x.setAttribute('aria-pressed',active?'true':'false');}sync();}));input?.addEventListener('input',()=>{for(const x of buttons){if(x.dataset.noLabel==='1'){selected.delete(x.getAttribute(attr)||'');x.classList.remove('active');x.setAttribute('aria-pressed','false');}}sync();});}setup('sexualOrientationTags','data-orientation-tag','sexualOrientationProxy','sexualOrientationOther','self_orientation');setup('romanticOrientationTags','data-romantic-tag','romanticOrientationProxy','romanticOrientationOther','self_romantic_orientation');const posMap={'男性':0,'偏男性':17,'男性-非二元之间':33,'非二元':50,'非二元-女性之间':67,'偏女性':83,'女性':100,'轴外/不适用':'NA'};const syncGenderMeta=()=>{state.selfLikert.self_gender_tags=[...document.querySelectorAll('[data-gender-tag].active')].map(x=>x.dataset.genderTag).filter(Boolean);state.selfLikert.self_gender_other=(document.getElementById('genderOther')?.value||'').trim().slice(0,28);};document.querySelectorAll('[data-gender-position]').forEach(btn=>btn.addEventListener('click',()=>{state.selfLikert.self_gender_position=posMap[btn.dataset.genderPosition]??'NA';state.selfLikert.self_gender_position_label=btn.dataset.genderPosition||'';}));document.querySelectorAll('[data-gender-tag]').forEach(btn=>btn.addEventListener('click',syncGenderMeta));document.getElementById('genderOther')?.addEventListener('input',syncGenderMeta);})();</script>`;
...
function validV33Scores(scores) {
  ...
  const requiredSelf = ['self_romantic_orientation','self_rom_m','self_rom_f','self_rom_nb','self_gender_expression','self_phys_m','self_phys_f','self_phys_nb','self_phys_intensity','self_libido','self_romantic_desire','self_relationship_openness','self_multi_partner'];
...
function patchMain(html) {
  ...
  html = html.replace('<div class="field"><label>8. 你的日常性别表达更接近哪里？</label>', `${TARGET_SELF_FIELDS}<div class="field"><label>10. 你的日常性别表达更接近哪里？</label>`);
